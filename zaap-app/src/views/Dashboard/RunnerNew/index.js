...
function RunnerNew() {
  ...
  const initialValues = useMemo(() => ({
    ...
    //   + Math.random().toString(36).slice(2)
    token: 'xpkjzod3wlo7vt2n5qhaz93qmc4zzc3ru'
  }), [])
        ...
        <Form
          ...
          render={({ handleSubmit, pristine }) => (
            <form onSubmit={handleSubmit}>
              <Field component={TextField} name="name" label="Name" placeholder="my-runner" required/>
              <Field component={TextField} name="description" label="Description"/>
              <Field component={TextField} name="url" label="URL" placeholder="localhost:8090" required/>
              <Field component={TextField} name="token" label="Token" disabled required/>
              <Button loading={isLoading} disabled={pristine} className="btn btn-success" type="submit">
                Register runner
              </Button>
            </form>
          )}
